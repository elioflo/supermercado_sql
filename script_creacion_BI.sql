...
JOIN [LOS_REZAGADOS].BI_dimension_categorias DC
  ON SC.subcategoria_descripcion = DC.subcategoria_descripcion
  AND C.categoria_descripcion = DC.categoria_descripcion
GROUP BY
  YEAR(TV.ticket_fecha_hora),
  [LOS_REZAGADOS].fn_GetCuatrimestre(TV.ticket_fecha_hora),
...
JOIN [LOS_REZAGADOS].BI_dimension_ubicaciones U
  ON L.localidad_descripcion = U.localidad_descripcion
  AND P.provincia_descripcion = U.provincia_descripcion
...
SELECT anio,
  mes,
  localidad_descripcion,
  SUM(ticket_sub_total) / SUM(cantidad_ventas) AS TotalVentas
...
SELECT DISTINCT
    ...
    Ti.anio AS anio,
    SUM(V.ticket_detalle_cantidad) / SUM(V.cantidad_ventas) AS unidades_promedio
...
SELECT
    ...
    SUM(V.cantidad_ventas) AS ventas
FROM [LOS_REZAGADOS].BI_hechos_ventas V
...
SELECT
  ...
  cuatrimestre,
  rango_descripcion,
  COUNT(hechos_envios_id) AS CantidadEnvios
...
SELECT AVG(importe/detalle_cuotas) AS Promedio , rango_id
...
SELECT
    ...
    T.cuatrimestre,
    (SUM(V.ticket_total_descuento_medio_pago)/SUM(V.ticket_sub_total)) * 100 AS Porcentaje_Descuento_Aplicado
FROM [LOS_REZAGADOS].BI_hechos_pagos P
JOIN [LOS_REZAGADOS].BI_dimension_tiempos T ON P.tiempo_id = T.tiempo_id
JOIN [LOS_REZAGADOS].BI_hechos_ventas V ON T.tiempo_id = V.tiempo_id
JOIN [LOS_REZAGADOS].BI_dimension_medios_de_pago MP ON P.medios_de_pago_id = MP.medios_de_pago_id
